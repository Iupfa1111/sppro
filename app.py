...
if "usuarios" not in st.session_state:
    st.session_state["usuarios"] = {
        ...
        "operador1": {"pass": "user123", "activo": True, "rol": "Operador"}
    }
...
if "edificios" not in st.session_state:
    st.session_state["edificios"] = [
        {
            "id": 1,
            "nombre": "Edificio Central / Planta",
            "direccion": "Av. Corrientes 1000, CABA",
            "descripcion": "Entrada principal sobre avenida, salida de emergencia por calle lateral.",
            "adjunto": "Sin adjunto",
            "privado": False
        }
    ]
...
if not st.session_state["logged_in"]:
    ...
    with col2:
        ...
        if st.button("Ingresar"):
            ...
            if usuario in db and db[usuario]["pass"] == clave:
                if db[usuario]["activo"]:
                    st.session_state["logged_in"] = True
                    st.session_state["user_role"] = db[usuario]["rol"]
                    st.rerun()
                else:
...
st.sidebar.caption(f"Operador activo: {st.session_state['user_role']}")

opciones = ["🗺️ Planificación de Rutas", "🏢 Entradas/Salidas de Edificios"]
...
if seccion == "🗺️ Planificación de Rutas":
    ...
    if loc and 'coords' in loc:
        ...
        st.success(f"📍 GPS Activo: {lat_gps:.4f}, {lon_gps:.4f}")
    ...
    if coords_orig and coords_dest:
        ...
        folium.Marker(
            ...
            popup="Comisaría Cercana",
            icon=folium.DivIcon(html='<div style="font-size:14pt; color:white; background-color:blue; border-radius:50%; text-align:center; width:28px; height:28px; line-height:28px; font-weight:bold;">C</div>')
        ).add_to(m)

        folium.Marker(
            [coords_dest[0] - 0.002, coords_dest[1] - 0.002],
            popup="Hospital / Urgencias",
            icon=folium.DivIcon(html='<div style="font-size:14pt; color:white; background-color:red; border-radius:50%; text-align:center; width:28px; height:28px; line-height:28px; font-weight:bold;">H</div>')
        ).add_to(m)

        st_folium(m, width=1100, height=480)

        st.divider()
        st.subheader("🛣️ Hoja de Ruta Terrestre (Calles a transitar)")
        if pasos_calles:
    ...
    with st.form("form_edificio"):
        ...
        es_privado = st.checkbox("🔒 Marcar como exclusivo/privado (Solo visible para Administrador)")

        if st.form_submit_button("Guardar Edificio"):
            if nombre_edf and direccion_edf:
                origen_adjunto = "Sin adjunto"
                if foto_camara:
                    origen_adjunto = "Foto capturada con la cámara"
                elif foto_subida:
                    origen_adjunto = f"Archivo: {foto_subida.name}"

                nuevo_id = len(st.session_state["edificios"]) + 1
                st.session_state["edificios"].append({
                    "id": nuevo_id,
                    "nombre": nombre_edf,
                    "direccion": direccion_edf,
                    "descripcion": desc_accesos,
                    "adjunto": origen_adjunto,
                    "privado": es_privado
                })
    ...
    lista_edificios = st.session_state["edificios"]
    if not es_admin:
        lista_edificios = [e for e in lista_edificios if not e["privado"]]
        ...
        for edf in lista_edificios:
            ...
            with st.expander(f"🏢 {edf['nombre']} - {edf['direccion']} | [{estado_vis}]"):
                ...
                if es_admin:
                    st.divider()
                    if edf["privado"]:
                        if st.button("Hacer Público", key=f"pub_{edf['id']}"):
                            edf["privado"] = False
                            st.rerun()
                    else:
                        if st.button("Hacer Privado", key=f"priv_{edf['id']}"):
                            edf["privado"] = True
                            st.rerun()
    else:
    ...
    with st.form("nuevo_usuario"):
        ...
        if st.form_submit_button("Crear Usuario"):
            if nuevo_user and nueva_pass:
                st.session_state["usuarios"][nuevo_user] = {"pass": nueva_pass, "activo": True, "rol": rol}
                st.success(f"✅ Usuario '{nuevo_user}' creado exitosamente.")

    st.divider()
    st.subheader("⚙️ Control de Estado de Usuarios")
    for u, datos in st.session_state["usuarios"].items():
        if u != "admin":
            ...
            if datos["activo"]:
                ...
                if col_btn.button("Reactivar", key=f"act_{u}"):
                    st.session_state["usuarios"][u]["activo"] = True
                    st.rerun()
